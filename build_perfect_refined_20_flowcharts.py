...
def build_grid_free_svg(p):
  ...
  <!-- Clean, Solid Dark Background (NO GRID OVERLAY) -->
  <rect width="100%" height="100%" fill="#0d1117" rx="12"/>

  <!-- Header Text -->
  <text x="500" y="32" text-anchor="middle" fill="#ffffff" font-family="Inter, -apple-system, sans-serif" font-weight="700" font-size="20">Project {p['num']}: {p['title']}</text>
  <text x="500" y="54" text-anchor="middle" fill="#8b949e" font-family="Inter, -apple-system, sans-serif" font-size="13">2D Control Flow Architecture Blueprint</text>
  ...
  <g transform="translate(320, 150)">
    ...
    <text x="180" y="22" text-anchor="middle" fill="#ffffff" font-family="Inter, sans-serif" font-weight="600" font-size="13">{p['step1_title']}</text>
  ...
  <g transform="translate(500, 275)">
    <polygon points="0,-40 180,0 0,40 -180,0" fill="#1f1906" stroke="#fbbf24" stroke-width="2.5" filter="url(#glow-gold)"/>
    <text x="0" y="-10" text-anchor="middle" fill="#fbbf24" font-family="Inter, sans-serif" font-weight="700" font-size="11">DECISION 1</text>
    <text x="0" y="8" text-anchor="middle" fill="#ffffff" font-family="Inter, sans-serif" font-weight="600" font-size="11">{p['d1_cond']}</text>
  ...
  <g transform="translate(30, 350)">
    ...
    <text x="140" y="22" text-anchor="middle" fill="#ffffff" font-family="Inter, sans-serif" font-weight="600" font-size="12">{p['left_action_title']}</text>
  ...
  <g transform="translate(320, 350)">
    <rect width="360" height="50" rx="8" fill="#161b22" stroke="#38bdf8" stroke-width="1.5"/>
    <text x="180" y="22" text-anchor="middle" fill="#ffffff" font-family="Inter, sans-serif" font-weight="600" font-size="13">{p['step2_title']}</text>
  ...
  <g transform="translate(500, 485)">
    ...
    <text x="0" y="8" text-anchor="middle" fill="#ffffff" font-family="Inter, sans-serif" font-weight="600" font-size="11">{p['d2_cond']}</text>
  ...
  <g transform="translate(320, 565)">
    ...
    <text x="180" y="22" text-anchor="middle" fill="#ffffff" font-family="Inter, sans-serif" font-weight="600" font-size="13">{p['success_action_title']}</text>
  ...
  <g transform="translate(840, 565)">
    ...
    <text x="0" y="10" text-anchor="middle" fill="#ffffff" font-family="Inter, sans-serif" font-size="10">{p['d3_cond']}</text>
  </g>
  ...
  <g transform="translate(710, 645)">
    ...
    <text x="130" y="22" text-anchor="middle" fill="#ffffff" font-family="Inter, sans-serif" font-weight="600" font-size="12">{p['fail_action_title']}</text>
    <text x="130" y="38" text-anchor="middle" fill="#f43f5e" font-family="JetBrains Mono, monospace" font-size="10">{p['fail_action_sub']}</text>
  </g>
</svg>"""

def build_refined_html(p):
...
<head>
    ...
    <style>
        ...
        * {{
        ...
        /* Executive Decision Architecture Grid */
        .exec-grid {{
            display: grid;
...
<body>
    ...
    <!-- Clean, Grid-Free Vector SVG Diagram Container -->
    <div class="diagram-container">
        <img src="FLOWCHART.svg" alt="2D Control Flow Architecture Diagram">
    </div>
...
print("1. Regenerating all 20 FLOWCHART.svg files with solid dark canvas (NO GRID OVERLAY)...")
for p in projects_data:
    svg_content = build_grid_free_svg(p)
    svg_path = os.path.join(base_dir, p["dir"], "FLOWCHART.svg")
    with open(svg_path, "w") as f:
        f.write(svg_content)
    print(f"Created grid-free SVG: {p['dir']}/FLOWCHART.svg")

print("2. Rewriting all 20 FLOWCHART.html files with executive architecture cards...")
for p in projects_data:
    html_content = build_refined_html(p)
    html_path = os.path.join(base_dir, p["dir"], "FLOWCHART.html")
    with open(html_path, "w") as f:
        f.write(html_content)
    print(f"Rewrote FLOWCHART.html: {p['dir']}/FLOWCHART.html")

print("3. Copying updated SVG and HTML files to 5 secondary alias directories...")
alias_mappings = [
...
for src_dir, target_dir in alias_mappings:
    ...
    src_html = os.path.join(base_dir, src_dir, "FLOWCHART.html")
    target_html = os.path.join(base_dir, target_dir, "FLOWCHART.html")
    with open(src_html, "r") as f_in:
        with open(target_html, "w") as f_out:
            f_out.write(f_in.read())
    print(f"Copied SVG & HTML from {src_dir} to {target_dir}")

print("Successfully refined and rewritten all 20 project FLOWCHART.svg and FLOWCHART.html files!")
