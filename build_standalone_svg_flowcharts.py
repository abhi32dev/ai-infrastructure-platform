...
def build_standalone_svg(p):
    return f"""<svg viewBox="0 0 1000 750" width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
  <defs>
  ...
  <pattern id="grid" width="40" height="40" patternUnits="userSpaceOnUse">
    <path d="M 40 0 L 0 0 0 40" fill="none" stroke="#141a24" stroke-width="1"/>
  </pattern>
  <rect width="100%" height="100%" fill="url(#grid)" />

  <!-- Diagram Title Header -->
  <text x="500" y="30" text-anchor="middle" fill="#f0f6fc" font-family="Inter, -apple-system, sans-serif" font-weight="700" font-size="20">Project {p['num']}: {p['title']}</text>
  <text x="500" y="52" text-anchor="middle" fill="#8b949e" font-family="Inter, -apple-system, sans-serif" font-size="13">2D Control Flow Architecture &amp; Decision Tree Blueprint</text>
  ...
  <g transform="translate(340, 75)">
    ...
    <text x="160" y="26" text-anchor="middle" fill="#34d399" font-family="Inter, sans-serif" font-weight="600" font-size="13">▶ {p['start_text']}</text>
  </g>
  ...
  <g transform="translate(320, 150)">
    <rect width="360" height="50" rx="8" fill="#12161f" stroke="#38bdf8" stroke-width="1.5"/>
    <text x="180" y="22" text-anchor="middle" fill="#f0f6fc" font-family="Inter, sans-serif" font-weight="600" font-size="13">{p['step1_title']}</text>
    <text x="180" y="38" text-anchor="middle" fill="#38bdf8" font-family="JetBrains Mono, monospace" font-size="11">{p['step1_code']}</text>
  </g>
  ...
  <g transform="translate(500, 275)">
    <polygon points="0,-40 180,0 0,40 -180,0" fill="#2d2206" stroke="#fbbf24" stroke-width="2" filter="url(#glow)"/>
  ...
  <text x="250" y="269" text-anchor="middle" fill="#34d399" font-family="JetBrains Mono, monospace" font-weight="600" font-size="10">{p['d1_left_label']}</text>
  ...
  <g transform="translate(40, 445)">
    ...
    <text x="130" y="25" text-anchor="middle" fill="#34d399" font-family="Inter, sans-serif" font-weight="600" font-size="12">{p['left_end_text']}</text>
  </g>

  <!-- DOWN BRANCH: Proceed Execution -->
  <path d="M 500 315 L 500 350" fill="none" stroke="#38bdf8" stroke-width="2" marker-end="url(#arrow-cyan)"/>
  <rect x="510" y="320" width="100" height="20" rx="4" fill="#12161f" stroke="#38bdf8" stroke-width="1"/>
  <text x="560" y="334" text-anchor="middle" fill="#38bdf8" font-family="JetBrains Mono, monospace" font-weight="600" font-size="10">{p['d1_down_label']}</text>

  <!-- Step 2 Box -->
  <g transform="translate(320, 350)">
    <rect width="360" height="50" rx="8" fill="#12161f" stroke="#38bdf8" stroke-width="1.5"/>
    <text x="180" y="22" text-anchor="middle" fill="#f0f6fc" font-family="Inter, sans-serif" font-weight="600" font-size="13">{p['step2_title']}</text>
    <text x="180" y="38" text-anchor="middle" fill="#38bdf8" font-family="JetBrains Mono, monospace" font-size="11">{p['step2_code']}</text>
  </g>
  ...
  <text x="560" y="544" text-anchor="middle" fill="#34d399" font-family="JetBrains Mono, monospace" font-weight="600" font-size="10">{p['d2_yes_label']}</text>

  <!-- Success Box -->
  <g transform="translate(320, 565)">
    <rect width="360" height="50" rx="8" fill="#12161f" stroke="#34d399" stroke-width="1.5"/>
    <text x="180" y="22" text-anchor="middle" fill="#f0f6fc" font-family="Inter, sans-serif" font-weight="600" font-size="13">{p['success_action_title']}</text>
    <text x="180" y="38" text-anchor="middle" fill="#34d399" font-family="JetBrains Mono, monospace" font-size="11">{p['success_action_sub']}</text>
  </g>
  ...
  <g transform="translate(340, 655)">
    <rect width="320" height="42" rx="21" fill="#092e20" stroke="#34d399" stroke-width="2"/>
    <text x="160" y="26" text-anchor="middle" fill="#34d399" font-family="Inter, sans-serif" font-weight="600" font-size="13">★ {p['end_success_text']}</text>
  </g>
  ...
  <text x="735" y="479" text-anchor="middle" fill="#f43f5e" font-family="JetBrains Mono, monospace" font-weight="600" font-size="10">{p['d2_no_label']}</text>

  <!-- Decision 3 Diamond (Right Side) -->
  <g transform="translate(840, 565)">
    <polygon points="0,-35 110,0 0,35 -110,0" fill="#2d2206" stroke="#fbbf24" stroke-width="2"/>
  ...
  <text x="770" y="369" text-anchor="middle" fill="#fbbf24" font-family="JetBrains Mono, monospace" font-weight="600" font-size="9">↻ {p['retry_loop_label']}</text>
  ...
  <text x="890" y="624" text-anchor="middle" fill="#f43f5e" font-family="JetBrains Mono, monospace" font-weight="600" font-size="10">{p['d3_no_label']}</text>

  <g transform="translate(710, 645)">
    <rect width="260" height="50" rx="8" fill="#12161f" stroke="#f43f5e" stroke-width="1.5"/>
    <text x="130" y="22" text-anchor="middle" fill="#f0f6fc" font-family="Inter, sans-serif" font-weight="600" font-size="12">{p['fail_action_title']}</text>
    <text x="130" y="38" text-anchor="middle" fill="#f43f5e" font-family="JetBrains Mono, monospace" font-size="10">{p['fail_action_sub']}</text>
  </g>
</svg>"""
...
print("1. Removing all PNG flowchart images across project directories...")
for png_file in glob.glob(os.path.join(base_dir, "*", "FLOWCHART.png")):
    try:
        os.remove(png_file)
        print(f"Removed PNG: {png_file}")
    except Exception as e:
        print(f"Failed to remove {png_file}: {e}")

print("2. Generating standalone FLOWCHART.svg vector files for all 20 projects...")
for p in projects_spec:
    svg_content = build_standalone_svg(p)
    svg_path = os.path.join(base_dir, p["dir"], "FLOWCHART.svg")
    with open(svg_path, "w") as f:
        f.write(svg_content)
    print(f"Created standalone SVG: {p['dir']}/FLOWCHART.svg")

print("3. Embedding FLOWCHART.svg directly into FLOWCHART.html and README.md files...")
all_dirs = [p["dir"] for p in projects_spec] + [
    "02-agentic-workflow-engine", "03-high-throughput-rag-engine",
    "04-realtime-stream-feature-pipeline", "05-ml-observability-monitoring-stack",
    "06-auto-scaling-inference-gateway"
]

for d in all_dirs:
    # Also copy SVG to alias directories
    if "agentic-workflow" in d:
        src_svg = os.path.join(base_dir, "02-rag-cost-router", "FLOWCHART.svg")
        target_svg = os.path.join(base_dir, d, "FLOWCHART.svg")
        with open(src_svg, "r") as f_in:
            with open(target_svg, "w") as f_out:
                f_out.write(f_in.read())
    elif "high-throughput-rag" in d:
        src_svg = os.path.join(base_dir, "03-llm-eval-gate", "FLOWCHART.svg")
        target_svg = os.path.join(base_dir, d, "FLOWCHART.svg")
        with open(src_svg, "r") as f_in:
            with open(target_svg, "w") as f_out:
                f_out.write(f_in.read())
    elif "realtime-stream" in d:
        src_svg = os.path.join(base_dir, "04-model-serving-mlops", "FLOWCHART.svg")
        target_svg = os.path.join(base_dir, d, "FLOWCHART.svg")
        with open(src_svg, "r") as f_in:
            with open(target_svg, "w") as f_out:
                f_out.write(f_in.read())
    elif "ml-observability" in d:
        src_svg = os.path.join(base_dir, "05-event-stream-pyspark-etl", "FLOWCHART.svg")
        target_svg = os.path.join(base_dir, d, "FLOWCHART.svg")
        with open(src_svg, "r") as f_in:
            with open(target_svg, "w") as f_out:
                f_out.write(f_in.read())
    elif "auto-scaling-inference" in d:
        src_svg = os.path.join(base_dir, "06-finetuning-lora-alignment", "FLOWCHART.svg")
        target_svg = os.path.join(base_dir, d, "FLOWCHART.svg")
        with open(src_svg, "r") as f_in:
            with open(target_svg, "w") as f_out:
                f_out.write(f_in.read())

    # Update FLOWCHART.html
    html_path = os.path.join(base_dir, d, "FLOWCHART.html")
    if os.path.exists(html_path):
        with open(html_path, "r") as f:
            html = f.read()

        # Remove old PNG image tags or base64 blocks
        svg_img_tag = '''
    <div style="text-align:center; margin-bottom:2.5rem;">
        <div style="font-size:0.85rem; color:#38bdf8; margin-bottom:0.75rem; font-weight:600; letter-spacing:0.05em;">INFINITELY SCALABLE VECTOR ARCHITECTURE DIAGRAM (FLOWCHART.svg)</div>
        <img src="FLOWCHART.svg" alt="2D Control Flow Architecture Diagram" style="width:100%; max-width:1300px; height:auto; border-radius:16px; border:2px solid #21262d; box-shadow: 0 12px 36px rgba(0,0,0,0.7);">
    </div>
'''
        ...
        if '<div class="section-title">' in html:
            ...
            if '<span>⚡ Exhaustive' in rest:
                ...
                with open(html_path, "w") as f:
                    f.write(new_html)
                print(f"Updated {d}/FLOWCHART.html to embed FLOWCHART.svg")
    ...
    if os.path.exists(readme_path):
        ...
        # Replace FLOWCHART.png references with FLOWCHART.svg
        content = content.replace("FLOWCHART.png", "FLOWCHART.svg")
        if "![2D Control Flow Diagram](FLOWCHART.svg)" not in content and "![2D Control Flow Architecture](FLOWCHART.svg)" not in content:
            img_md = "\n\n![2D Control Flow Architecture](FLOWCHART.svg)\n\n"
            if "\n---\n" in content:
        ...
        with open(readme_path, "w") as f:
            f.write(content)
        print(f"Updated {d}/README.md to embed FLOWCHART.svg")

print("Successfully generated standalone FLOWCHART.svg vector files and removed all PNG files!")
